...
lapply(1:1, function(i) { 
  radarchart(rbind(rep(1,1), rep(0,1), one.team[i,-1]), pdensity=c(0, 0, 100), 
             title=expression(paste(one.team[i, 1], "Team Hitting (vs. Lefties)")), maxmin=F, axistype=0, centerzero=T, vlabels=offense.labels, vlcex=1)
})